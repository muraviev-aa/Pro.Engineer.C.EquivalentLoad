...
void equivalentLoadFromFreeLocalLoad(void); // эквивалентная от сосредоточенной силы в середине пролета
...
void equivalentLoadFromFreeLocalLoad()
{
    ...
    scanf("%Lf", &beamLength);
    fflush(stdin);
    puts("Введите значение привязки сосредоточенной силы к левой опоре балки, м");
